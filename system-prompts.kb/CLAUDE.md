...
Shapes are model-scoped since v2.1.221, so variants are named by model class:
unsuffixed = the long-form `# System` shape (served to sonnet-5), `-opus` and
`-fable` = the two distinct `# Harness` shapes (opus gets `# Delivering
work`/`# Corrections`, fable gets `# Communicating with the user`). Earlier
