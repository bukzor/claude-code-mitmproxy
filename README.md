...
| `syspatch.py` | `prompt_location`, `prompt_patches` | Rewrites the system prompt using modular patch directories from `~/.config/claude-mitmproxy/system-prompt.d/` (format and rationale in that directory's README). A patch whose in-scope target has drifted warns loudly and captures the offending body under `log/patch-failures/` for triage — see `CLAUDE.kb/patch-failure-triage.md`. |
| `toolpatch.py` | `tool_patches` | Replaces built-in tool descriptions with slim stubs from `~/.config/claude-mitmproxy/tool-description.d/` (format in that directory's README). Upstream drift warns loudly and captures like a syspatch failure. |
