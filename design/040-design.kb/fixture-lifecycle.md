...
1. `syscapture.py` writes each unique pre-patch prompt body to
   ...
   `v{cc_version}_{model}_{digest}.md` — automatic, deduplicated by
   masked digest.
2. A human promotes noteworthy captures into `system-prompts.kb/`
   under that collection's naming rules (version, `-variant`, `-scope`).
