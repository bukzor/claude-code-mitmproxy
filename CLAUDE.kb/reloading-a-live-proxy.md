...
`from claude_mitmproxy import syspatch` and gets an ordinary second copy.
Re-executing the addon copy leaves the library copy stale. The two copies
are structural, not a packaging accident -- a path-loaded addon is never
the module any importer can name, whatever the package layout.
